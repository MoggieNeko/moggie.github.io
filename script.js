...
function generateFinalResults() {
  ...
  // 首先統計名字重複情況（全局和每個池）
  let playerNameCounts = {}; // 全局計數
  let poolPlayerNameCounts = {}; // 每個池的計數
  
  // 初始化池內名字計數器
  for (let i = 1; i <= globalPoolTotal; i++) {
    let poolID = String.fromCharCode(64 + i);
    poolPlayerNameCounts[poolID] = {};
  }
  ...
  for (let i = 1; i <= globalPoolTotal; i++) {
    ...
    rows.forEach(row => {
      ...
      // 計數
      playerNameCounts[playerName] = (playerNameCounts[playerName] || 0) + 1;
      poolPlayerNameCounts[poolID][playerName] = (poolPlayerNameCounts[poolID][playerName] || 0) + 1;
    });
  }
  
  // 第二階段：收集每個池中的每位選手，並添加到映射表中
  for (let i = 1; i <= globalPoolTotal; i++) {
    let poolID = String.fromCharCode(64 + i);
    let poolDiv = document.getElementById("pool-" + poolID);
    if (!poolDiv) continue;
    ...
    rows.forEach((row, rowIndex) => {
      ...
      // 計算同一池內同名選手的索引
      inPoolNameIndexes[playerName] = (inPoolNameIndexes[playerName] || 0) + 1;
      const nameIndex = inPoolNameIndexes[playerName];
      
      // 建立選手的唯一ID和顯示名稱
      const uniqueId = `${playerName}_${poolID}_${nameIndex}`;
      let displayName = playerName;
      
      // 生成顯示名稱（處理重名情況）
      if (poolPlayerNameCounts[poolID][playerName] > 1) {
        // 同一池中有重名，添加編號
        displayName = `${playerName} (${poolID}-${nameIndex})`;
      } else if (playerNameCounts[playerName] > 1) {
        // 不同池中有重名，添加池號
        displayName = `${playerName} (Pool ${poolID})`;
      }
      ...
      playersMap[uniqueId] = {
        ...
        place: place,
        DQ: isDQ,
        wasInKnockout: false, // 默認未參加淘汰賽
        eliminatedRound: -1, // 默認沒有淘汰輪次
        originalIndex: rowIndex + 1, // 原始行索引
        uniqueId: uniqueId
      };
    });
  }
    ...
    round.forEach(match => {
      ...
      if (match.player1 && match.player1.name !== "BYE") {
        ...
        for (let id in playersMap) {
          let p = playersMap[id];
          if (p.name === match.player1.name && p.pool === player1PoolID) {
            // 找到匹配的選手，更新淘汰賽信息
            p.wasInKnockout = true;
            p.seeding = match.player1.seeding;
            foundInPool = true;
            break;
          }
        }
        ...
        // 如果沒有找到（特殊情況），添加新記錄
        if (!foundInPool) {
          const uniqueId = `${match.player1.name}_${player1PoolID}_0`;
          playersMap[uniqueId] = {
            name: match.player1.name,
            displayName: match.player1.name,
            pool: player1PoolID,
            wasInKnockout: true,
            seeding: match.player1.seeding,
            place: 0,
            DQ: false,
            eliminatedRound: -1,
            uniqueId: uniqueId
      ...
      if (match.player2 && match.player2.name !== "BYE") {
        ...
        for (let id in playersMap) {
          let p = playersMap[id];
          if (p.name === match.player2.name && p.pool === player2PoolID) {
            // 找到匹配的選手，更新淘汰賽信息
            p.wasInKnockout = true;
            p.seeding = match.player2.seeding;
            foundInPool = true;
            break;
          }
        }
        ...
        // 如果沒有找到，添加新記錄
        if (!foundInPool) {
          const uniqueId = `${match.player2.name}_${player2PoolID}_0`;
          playersMap[uniqueId] = {
            name: match.player2.name,
            displayName: match.player2.name,
            pool: player2PoolID,
            wasInKnockout: true,
            seeding: match.player2.seeding,
            place: 0,
            DQ: false,
            eliminatedRound: -1,
            uniqueId: uniqueId
      ...
      if (match.loser && match.loser.name !== "BYE") {
        for (let id in playersMap) {
          let p = playersMap[id];
          if (p.name === match.loser.name && p.pool === (match.loser.pool || "")) {
  ...
  allPlayers.sort((a, b) => {
    ...
    // 其次，淘汰賽選手排在僅參加小組賽選手之前
    if (a.wasInKnockout && !b.wasInKnockout) return -1;
    if (!a.wasInKnockout && b.wasInKnockout) return 1;
    
    // 對於淘汰賽選手，按照淘汰輪次排序
    if (a.wasInKnockout && b.wasInKnockout) {
  ...
  finalTitle.innerHTML = `<h2>【${globalTournamentName}】 Final Results</h2>`;
  ...
  allPlayers.forEach((player, index) => {
    ...
    tr.innerHTML = `<td>${rankText}</td><td>${player.displayName}</td><td>${player.pool || ""}</td>`;
 ...
 window.nextPowerOf2 = nextPowerOf2;
  ...
  function switchPage(pageId) {
    ...
    if (selectedPage) {
      ...
      // 如果切換到個人資料頁面，重新載入用戶資料
      if (pageId === 'profilePage' && typeof loadUserProfileData === 'function') {
        loadUserProfileData();
      } else if (typeof updateUserDisplay === 'function') {
        // 對於其他頁面，至少更新用戶顯示，確保頭像保持一致
        updateUserDisplay();
      }
        } else {
    ...
    switch(pageId) {
      ...
      case "profilePage": navIndex = 5; break;
      case "settingsPage": navIndex = 6; break;
    }
