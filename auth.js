...
async function updateUserDisplay() {
  ...
  if (userNameElement && currentUser) {
    ...
    if (currentUser.avatar_url) {
      try {
        // 檢查是否有手動更新標記，如果有則跳過自動更新
        if (window.manualAvatarUpdate) {
          console.log('檢測到手動更新標記，跳過頭像自動更新');
          return;
        }
        ...
        if (profileAvatar) {
          // 檢查當前src是否已經是最新的URL（去除時間戳部分）
          const currentSrc = profileAvatar.src ? profileAvatar.src.split('?')[0] : '';
          const newSrc = avatarUrl.split('?')[0];
    ...
    if (avatarUrl) {
      try {
        ...
        if (urlError) {
          ...
          if (avatarPreview) {
            // 設置一個標記表示這是手動更新，避免被其他代碼重複更新
            window.manualAvatarUpdate = true;
            ...
            avatarPreview.onload = function() {
              ...
              // 5秒後清除標記
              setTimeout(() => {
                window.manualAvatarUpdate = false;
              }, 5000);
            };
